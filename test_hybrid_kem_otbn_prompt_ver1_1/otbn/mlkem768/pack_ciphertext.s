...
.globl poly_compress
.globl polyvec_compress
.globl pack_ciphertext
.globl poly_decompress
.globl polyvec_decompress
...
/**
 * ver0_2 接口: polyvec_compress —— u 多项式向量(k = 3)压缩并序列化
 * (du = 10)，输出 3 × 320 = 960 字节。
 *
 * @param[in]  x10: u 多项式向量起点 (3 × 1024 B, 32-bit 系数布局) [原地被压缩覆盖]
 * @param[out] x12: 输出 960 压缩字节
 *
 * Clobbered: x2, x3
 */
polyvec_compress:
...
/**
 * ver0_2 接口: pack_ciphertext —— ct = Compress(polyvec b) || Compress(v)
 * 共 1088 字节 (u: 960 B, v: 128 B)。
 *
 * @param[in]  x10: dptr_b, u 多项式向量 (3 × 1024 B) [调用后内容被压缩覆盖]
 * @param[in]  x11: dptr_v, v 多项式 (1024 B)        [同上]
 * @param[out] x12: dptr_output, 密文 (1088 B)
 */
pack_ciphertext:
  .irp reg, x10, x11, x12
    sw \reg, 0(x31)
...
/**
 * ver0_2 接口: polyvec_decompress —— u 解序列化并解压缩 (du = 10)，
 * 输入 960 字节，输出 3 × 1024 字节。
 *
 * @param[in]  x10: 输入 960 压缩字节
 * @param[out] x12: 输出 u 多项式向量 (3 × 1024 B)
 *
 * Clobbered: x2, x3
 */
polyvec_decompress:
...
/**
 * ver0_2 接口: unpack_ciphertext —— 密文(1088 B) -> u 向量(3 × 1024 B) + v(1024 B)
 *
 * @param[in]  x10: 输入密文 (1088 B)
 * @param[out] x12: 输出 u 多项式向量 (3 × 1024 B)
 * @param[out] x13: 输出 v 多项式 (1024 B)
 */
unpack_ciphertext:
  .irp reg, x10, x12, x13
    sw \reg, 0(x31)
