/*
 * ver1_1 对照：keygen_basemul_basemul_acc（与 profiling 逐条对应，仅少 9 条 jal）
 *
 * Δcycles = keygen_basemul_basemul_acc_profiling − 本目标。
 */
.section .text.start
...
main:
  ...
  /* 与 profiling 相同的缩放调用操作数准备（×3），但不发起调用 */
  la   x2, poly_s_hat
...
/* Montgomery 缩放常数：每 lane 2988（= 2^64 mod q） */
.balign 32
