...
/*
 * ver1_1 (ML-KEM-768, 官方向量指令实现) —— Decaps 测试/芯片仿真入口 + 内存布局
 *
 * 内存符号与输入向量 (ct/dk) 均取自 ver0_2，可直接复用其期望向量
 * (dec.dexp: ss 32 B)。
 */
...
/* ---- 再加密(encap 内核)所需的输入/状态缓冲 ----
 * decaps 会链接 mlkem_encap.s 做 K-PKE.Encrypt 再加密，因此需要这些缓冲。
 * 其中 poly_slot_y_hat 放在 .data 而非 .scratchpad：scratchpad 16KB 已被
 * decrypt + decaps 的工作缓冲占满（见 README）。 */
.globl pk_t
pk_t:
  .zero 1152
