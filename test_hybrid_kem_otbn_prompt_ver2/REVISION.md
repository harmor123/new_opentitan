...
| v2.5 | 2026-06-04 | IKM 去掉 role, KMAC-KDF Expand 添加 info 支持 |
| v2.8 | 2026-06-04 | Phase 1/2 拆分: phase1_keygen, phase2_alice/bob |
| v3.0 | 2026-06-04 | 文档完善, ref/ 分为 phase1/phase2 子目录 |
| v3.2 | 2026-06-04 | KMAC-KDF info 支持完成: input_info_len 独立符号, 注释更新 |
...
- **process_cnt_q**: auto-permutation (StMsgFeed→StProcessing) 时设为 ProcessCycles，解决连续 KMAC 调用时 Keccak 处理周期跳过

### KMAC-KDF 汇编 (`otbn/kmac_kdf/kmac_kdf.s`)

- `kmac_kdf_extract`: IKM 不含 info (info 仅在 Expand 使用)
- `kmac_kdf_expand`: 支持 `input_info` + `input_info_len` 独立传入
- `input_lengths` 布局: +0=ctx, +4=sid, +8=okm_len

### 文件结构 (v3.0)

- `ref/` 分为 `phase1/` (p256_kat.py, gen_kat.py) 和 `phase2/` (kmac_kdf_kat_alice.py, kmac_kdf_kat_bob.py)
