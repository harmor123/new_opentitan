"""kmac_kdf_kat_bob.py — Phase 2 Bob KMAC-KDF KAT generator.
...
Bob OKM == Alice OKM (standard KEM correctness).
"""
...
# ss_e from Bob ECDH: d_bob * Q_alice (identical to Alice's)
ss_e = bytes.fromhex(
    "064cf1e6c03cfb667b49502837d4e73a"
    "a54c53d1884ef3925e6bf9d09a1c9926")
# ss_m from ML-KEM decap (identical to Alice's encap ss_m)
ss_m = bytes.fromhex(
...
print(f"// KMAC-KDF Phase2 Bob (okm_len={okm_len})")
print(f"// ss_e = {ss_e.hex()}")
print(f"// ss_m = {ss_m.hex()}")
print(f"// OKM = {okm.hex()} (Bob == Alice, standard KEM correctness)")
print()
