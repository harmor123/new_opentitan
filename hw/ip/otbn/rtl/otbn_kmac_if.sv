...
(
  ...
  // SECDED-encoded zero for clearing upper WSR words on first digest response.
  otbn_base_intg_word_t zero_intg_word;
  prim_secded_inv_39_32_enc u_zero_intg (
    .data_i(32'b0),
    .data_o(zero_intg_word)
  );
  ...
  for (genvar word = 0; word < BaseWordsPerWLEN; word++) begin : g_wsr
    if (word < 2) begin : g_rsp_update
      ...
      // Upper WSR words: clear on digest response to avoid X (uninitialised)
      // SECDED integrity errors when SW reads the full 256-bit WSR.
      assign ispr_kmac_data_s0_d[word] =
          sec_wipe_ispr_kmac_data_s0_i ? wipe_data[word]               :
          response_wr                  ? zero_intg_word                :
          ispr_kmac_data_s0_wr_i       ? ispr_kmac_data_s0_wdata[word] :
                                         ispr_kmac_data_s0_q[word];
      assign ispr_kmac_data_s1_d[word] =
          sec_wipe_ispr_kmac_data_s1_i ? wipe_data[word]               :
