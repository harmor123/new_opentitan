#!/bin/bash
# ML-KEM-768 Encaps（官方 xof.s 版）RTL-ISS co-sim
set -euo pipefail
SCRIPT_DIR="$(dirname "$(readlink -e "${BASH_SOURCE[0]}")")"
ROOT_DIR="$(readlink -e "$SCRIPT_DIR/../../..")"
fail() { echo >&2 "FAIL: $*"; exit 1; }
echo "=== mlkem768_encap OTBN co-sim ==="
BAZEL_TARGET="//test_hybrid_kem_otbn_prompt_ver0_2/otbn/mlkem768:mlkem768_encap"
ELF="$ROOT_DIR/bazel-bin/test_hybrid_kem_otbn_prompt_ver0_2/otbn/mlkem768/mlkem768_encap.elf"
...
if grep -q "Mismatch\|%Error" "$RUN_LOG"; then
  ...
  fail "mlkem768_encap RTL-ISS mismatch"
fi
echo "PASS"
