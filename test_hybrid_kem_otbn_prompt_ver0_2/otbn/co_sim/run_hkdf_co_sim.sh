#!/bin/bash
# HKDF-SHA3-256（官方 xof.s 版）RTL-ISS co-sim
set -euo pipefail
SCRIPT_DIR="$(dirname "$(readlink -e "${BASH_SOURCE[0]}")")"
ROOT_DIR="$(readlink -e "$SCRIPT_DIR/../../..")"
fail() { echo >&2 "FAIL: $*"; exit 1; }
echo "=== hkdf_sha3_256 OTBN co-sim ==="
BAZEL_TARGET="//test_hybrid_kem_otbn_prompt_ver0_2/otbn/hkdf:hkdf_sha3_256"
ELF="$ROOT_DIR/bazel-bin/test_hybrid_kem_otbn_prompt_ver0_2/otbn/hkdf/hkdf_sha3_256.elf"
...
if grep -q "Mismatch\|%Error" "$RUN_LOG"; then
  ...
  fail "hkdf_sha3_256 RTL-ISS mismatch"
fi
echo "PASS"
