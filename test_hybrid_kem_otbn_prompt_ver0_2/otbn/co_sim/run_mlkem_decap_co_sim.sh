#!/bin/bash
# ML-KEM-768 Decaps（官方 xof.s 版）RTL-ISS co-sim
set -euo pipefail
SCRIPT_DIR="$(dirname "$(readlink -e "${BASH_SOURCE[0]}")")"
ROOT_DIR="$(readlink -e "$SCRIPT_DIR/../../..")"
fail() { echo >&2 "FAIL: $*"; exit 1; }
echo "=== mlkem768_decap OTBN co-sim ==="
BAZEL_TARGET="//test_hybrid_kem_otbn_prompt_ver0_2/otbn/mlkem768:mlkem768_decap"
ELF="$ROOT_DIR/bazel-bin/test_hybrid_kem_otbn_prompt_ver0_2/otbn/mlkem768/mlkem768_decap.elf"
...
if grep -q "Mismatch\|%Error" "$RUN_LOG"; then
  ...
  fail "mlkem768_decap RTL-ISS mismatch"
fi
echo "PASS"
