...
indcpa_keypair:
  ...
  addi  x21, fp, -128
  addi  x20, x0, 1              /* Length 1 */
  addi  x22, x0, 0              /* Unmasked */
  jal   x1, xof_absorb

  jal   x1, xof_process

  /* Squeeze first 32 bytes (rho) to fp-128 */
  jal   x1, xof_squeeze32
  bn.xor w0, w29, w30
  addi  x5, x0, 0
  bn.sid x5, -128(x8)

  /* Squeeze next 32 bytes (sigma) to fp-96 */
  jal   x1, xof_squeeze32
  bn.xor w0, w29, w30
  addi  x5, x0, 0
  bn.sid x5, -96(x8)
...
crypto_kem_keypair: 
  ...
  /*** hash_h: SHA3-256 (official xof.s) ***/
  bn.xor w31, w31, w31
  jal   x1, xof_sha3_256_init

  /* Absorb pk (1184 bytes) */
  lw    x21, -32(fp)            /* Load pk pointer */
  addi  x20, x0, 1184           /* Length 1184 */
  addi  x22, x0, 0              /* Unmasked */
  jal   x1, xof_absorb
