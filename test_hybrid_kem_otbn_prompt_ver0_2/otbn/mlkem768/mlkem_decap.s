...
crypto_kem_dec:
  ...
  /*** hash_g(buf): SHA3-512 (official xof.s) ***/
  bn.xor w31, w31, w31
  jal   x1, xof_sha3_512_init

  li    x21, -4320
  add   x21, fp, x21     /* buf pointer (m' || H(pk)) */
  addi  x20, x0, 64      /* message length 64 bytes */
  ...
  addi  x5, x0, 0
  li    x10, -4256
  add   x10, fp, x10     /* output pointer (buf+64) */
  bn.sid x5, 0(x10)
  ...
  bn.xor w0, w29, w30
  addi  x5, x0, 0
  addi  x10, x10, 32     /* output pointer (buf+96) */
  bn.sid x5, 0(x10)
  ...
  /*** shake256(z||c, 32): SHAKE256 (official xof.s) ***/
  bn.xor w31, w31, w31
  jal   x1, xof_shake256_init

  lw    x21, -12(fp)     /* sk+2336 pointer */
  addi  x21, x21, 32     /* z pointer (sk+2368) */
  addi  x20, x0, 32      /* z length */
  addi  x22, x0, 0       /* unmasked */
  jal   x1, xof_absorb

  lw    x21, -20(fp)     /* ct pointer */
  li    x20, 1088        /* ct length */
  addi  x22, x0, 0       /* unmasked */
  jal   x1, xof_absorb

  jal   x1, xof_process

  /* squeeze 32B to buf+96 */
  jal   x1, xof_squeeze32
  bn.xor w0, w29, w30
  addi  x5, x0, 0
  li    x10, -4256
