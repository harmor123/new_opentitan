...
crypto_kem_enc: 
  ...
  /*** hash_h(pk): SHA3-256 (official xof.s) ***/
  bn.xor w31, w31, w31
  jal   x1, xof_sha3_256_init

  lw    x21, -24(fp)     /* x21 = pk pointer */
  addi  x20, x0, 1184    /* pk length */
  addi  x22, x0, 0       /* unmasked */
  jal   x1, xof_absorb

  jal   x1, xof_process

  /* squeeze 32B to fp-1088 (immediately after randombytes) */
  ...
  /*** hash_g(randombytes||hash_h(pk)): SHA3-512 (official xof.s) ***/
  /* Note: at fp-1120 there are exactly 32 bytes randombytes + 32 bytes hash_h(pk) = 64 bytes */
  bn.xor w31, w31, w31
  jal   x1, xof_sha3_512_init

  addi  x21, fp, -1120   /* message pointer (randombytes || hash_h(pk)) */
