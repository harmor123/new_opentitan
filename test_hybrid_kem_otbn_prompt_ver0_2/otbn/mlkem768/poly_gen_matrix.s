 ...
 *
 * clobbered registers: x5-x7, x10-x25, x27-x30, w0-w1, w8-w14, w26-w31
...
_aligned:
   ...
   *   fp - 32: squeeze output buffer (xof_squeeze32 output stored here: fp-32..fp-1)
  ...
  /* ─── Initialize SHAKE128 via official xof.s driver ─── */
  /* Save x10 (seed pointer) — clobbered by xof_absorb */
  sw   x10, -40(fp)

  /* Save x28-x30: official xof.s uses them for rate tracking / timeout,
   * and the caller keeps x29 (next-sk pointer) live across this call. */
  sw   x28, -48(fp)
  sw   x29, -52(fp)
  sw   x30, -56(fp)
...
_rej_sample_loop:
   ...
   * xof_squeeze32 clobbers: x25, x28-x30, w27, w29-w30
   * (rate-boundary RUN handled internally via x28 tracking)
   ...
   * Use x6 (WDR index, set each time — safe since xof_squeeze32 clobbers it anyway)
   * Use x13 (base addr, free throughout) */
  addi x13, fp, -96
  li   x6, 10
  bn.sid x6, 0(x13)

  /* Squeeze 32B (official xof.s) — block boundaries handled automatically */
...
_end_rej_sample_loop:
  ...
  /* Restore x28-x30 (saved before the XOF session) */
  lw   x28, -48(fp)
  lw   x29, -52(fp)
  lw   x30, -56(fp)
