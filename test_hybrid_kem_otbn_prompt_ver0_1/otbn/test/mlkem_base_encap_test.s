...
/* Stack lives in .scratchpad (NOLOAD -> not loaded through the mailbox).
 * 8 KiB, same as ver0_2; deepest frame chain is < 1 KiB. */
.section .scratchpad
.balign 32
.global stack
stack:
...
/* rc 每个常量占满 32 B 槽（低 64 位=常量，高 192 位=0）：keccakf 的 IOTA 步
   每轮整读 32 B，含最后一槽 ⇒ 表尾必须补齐到 32 B。否则当 rc 是已下装 DMEM
   镜像的最后一个符号时，最后那个整字读越过镜像 ⇒ DMEM_INTG_VIOLATION（中止）。
   本 app 当前靠"rc 之后还有别的数据段"侥幸不越界，这里补上以去掉隐患。 */
  .balign 32