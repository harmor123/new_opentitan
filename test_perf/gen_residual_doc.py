...
`test_perf/stall_profile.py` 把 app 的拍数按 PC（退役 + 停滞）归因后：
...
用法: python3 test_perf/gen_residual_doc.py [--out <路径>]
"""
import argparse
import re
import sys
...
def main() -> int:
    ...
    L = ["# 残差构成（三版 × 三 op，全实测）", "",
         ...
         f"（工具 `test_perf/stall_profile.py`，md5 {sorted(md5s)[0][:8]}…；"
    ...
    L += ["",
          ...
          "复现：", "", "```bash",
          "python3 test_perf/stall_profile.py --version <版本> --op <keygen|encap|decap> \\",
