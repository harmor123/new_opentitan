...
用法:
    python3 test_perf/fix_controls.py --version ver0_1 [--apply]
    （不带 --apply 只报告；--apply 会写文件 + 把旧 control 备份到 test_perf/attic_controls/<ver>/）
"""
import argparse
import json
import re
...
ATTIC = REPO / "test_perf" / "attic_controls"
...
def main() -> int:
    ...
    if not args.apply:
        print("\n（未加 --apply，仅报告；加 --apply 写文件 + 备份到 test_perf/attic_controls/）")
        return 0
    ...
    for row, _m, _e, want in changed:
        ...
        shutil.copy2(c, attic / f"{row}_control.s")
        c.write_text(want, encoding="utf-8")
        print(f"  [改写] {row}_control.s（旧版存 attic）")

    build = pkg / "BUILD"
    shutil.copy2(build, attic / "BUILD")
