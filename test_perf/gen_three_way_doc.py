...
用法: python3 test_perf/gen_three_way_doc.py [--out <路径>] [--doc-version "2026-09-20 v4（三版全实测）"]
"""
import argparse
import json
import sys
...
def main() -> int:
    ...
    ap.add_argument("--extra", default=str(REPO / "test_perf/doc_fragments/three_way_extra.md"))
    args = ap.parse_args()
    ...
    L.append("")
    L.append(f"> 数据版本：**{args.doc_version}**；本文件由 `test_perf/gen_three_way_doc.py` "
    ...
    L.append("")
    L.append("> 逐 op 的两两深化对照见 `ver0_1_vs_ver0_2_对照表.md` 与 `ver0_2_vs_ver1_1_对照表.md`。")
    L.append("")
    ...
    out = Path(args.out)
    if out.exists():
        out.with_suffix(".bak.md").write_text(out.read_text(encoding="utf-8"), encoding="utf-8")
        print(f"[备份] {out.with_suffix('.bak.md').name}")
    out.write_text("\n".join(L) + "\n", encoding="utf-8")
