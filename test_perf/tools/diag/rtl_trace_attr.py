...
两个口径（都来自 trace 实测，不做任何"推"）：
  ...
  · **含被调拍**（incl）= 图上 `C2 − C1`：从**进入函数**到**离开函数**的整段。
                        **逐条指令解码**（ISS 自带 ISA 表）：`jal`/`jalr(rd=x1)` 压栈、
                        `ret`（`jalr x0, x1, 0`）出栈 ⇒ 每帧的进入/离开 cycle 都是 trace 里读出来的。
                        不含摊派；帧之间会重复（被调会被父帧包含）⇒ **不能相加**。
...
判据分两类（**别混**）
  · 完整性（硬判据，不通过就是数据错）：Σ`E` == 同一次运行的 chip `INSN_CNT`；
    Σ(`E`+`S`) + 空档 == trace 跨度。
...
用法:
  python3 test_perf/tools/diag/rtl_trace_attr.py --version ver0_1 --op keygen \
      ...
      --uart0 logs_hkem/ver0_1/rtl/test_mlkem_keypair_only.uart0.log \
...
def parse_trace(path: Path):
    ...
    返回 `(per_pc, ΣE, ΣS, Σ取指等待, 跨度, 其它行数, frames)`：
      · `per_pc[pc] = [退役, 停滞, 取指等待]`（自身拍口径，逐函数相加 == 跨度）
      · `frames = [(进入 PC, 进入 cycle, 离开 cycle), ...]` —— 调用栈实测（含被调口径，C2 − C1）
    """
    per, n_e, n_s, other = {}, 0, 0, 0
    first_c = last_c = last_pc = None
    stack, frames, pending_call = [], [], False
    with path.open("r", encoding="utf-8", errors="replace") as f:
        for line in f:
            ...
            if kind == "E":
                e[0] += 1
                n_e += 1
                # ── 调用帧实测：每帧拍数 = 离开 cycle − 进入 cycle（图上 C2 − C1）──────
    ...
    return per, n_e, n_s, sum(v[2] for v in per.values()), span, other, frames
...
def main() -> int:
    ...
    per, n_e, n_s, gaps, span, other, frames = parse_trace(Path(args.trace))
    ...
    # ── 含被调拍：**trace 实测帧**（每帧 = 被调函数首条退休 → 该帧 ret 退休）──────────
    incl, n_frames = {}, len(frames)
    ...
    L = [f"# RTL trace 逐函数归因 · {args.version}/{args.op}", "",
         ...
         f"| 调用帧实测（`jal`/`jalr`/`ret` 解码建栈） | {n_frames:,} 帧，Σ帧拍 "
         f"{sum(c1 - c0 for _p, c0, c1 in frames):,} | 含被调、帧间会重复；"
         f"顶层未闭合帧不计 | ✓ |", "",
         "## 与 ISS 的差（**信息列**，不是错误）", "",
    ...
    L += ["## 逐函数 RTL 真实周期数（含被调 = 图上 C2 − C1）", "",
          ...
          "|---|---:|---:|---:|---:|---:|---:|---:|"]
    ranked = sorted(rows, key=lambda x: -(incl.get(x[0], 0)))
    ...
    L += ["", "> ¹ **取指等待**=两条记录之间的空拍（fetch 未回），摊给上一条记录所在的函数 —— "
          ...
          f"> 「含被调」逐条来自 trace 实测帧（{n_frames:,} 帧，`jal/jalr/ret` 解码建栈），"
          "**不能相加**（被调会被父帧重复计入）；要「相加等于整段」用自身拍列。", ""]
        ...
        Path(args.json_out).write_text(json.dumps(
            {"version": args.version, "op": args.op, "trace": args.trace,
             ...
             "symbols": [{"name": n, "retire": r, "stall": s, "fetch_wait": g, "cycles": c,
                          "inclusive": incl.get(n)} for n, r, s, g, c in rows],
