...
生成的"按需求块数"完全一致；改完再用 `test_perf/tools/check/check_stream_shape.py` 复核
（不变量 B：replay 总数 == app 实测；C'：逐会话不越界）。

用法: python3 test_perf/tools/gen/patch_stub_overhead_rows.py [--apply] [--version ver0_1] [--version ver0_2]
"""
import argparse
import re
import sys
