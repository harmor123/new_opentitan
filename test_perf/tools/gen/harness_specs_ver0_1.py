"""ver0_1 的剖面实测行规格（供 test_perf/tools/gen/gen_harness.py 生成 .s）。

行名与 config 里原有的 reuse 行一致，但这里**在它所属的 op 里真实调用**
（次数 = 该 app 源码的静态调用次数）。

调用次数（ver0_1 app 源码静态计数，.rept / LOOPI 迭代数连乘）：
...
x15 = 5+10、unpack_sk_pk = 2），与 ver0_1 的 app 完全对应。
"""

XOF01 = ["sha3_shake.s", "keygen_sha3_context_rc_data.s"]   # ver0_1 的哈希 = 软件 Keccak

# ver0_1 内核调用范式（与 ver0_2 同构：ver0_2 是它的移植）（逐字取自 app 的调用点；Plantard 标量约定）
...
SPECS01 = [
    ...
    dict(op="encap", row="noise_x7", app="mlkem_encap",
         ...
         srcs=["poly.s", "cbd.s", "sha3_shake.s", "keygen_poly_getnoise_eta_1_data.s"],
         prelude=[], calls=_NOISE_CALLS, data=_NOISE_DATA),
    dict(op="encap", row="h_ek", app="mlkem_encap",
         note="encap 的 H(ek) = SHA3-256(ek 1184 B)：软件 Keccak，1 次 update（与 app 一致）。",
         srcs=list(XOF01), prelude=[],
         calls=[(1, ["la   x10, context", "li   x11, 32", "jal  x1, sha3_init",
                     "la   x10, context", "la   x11, input_ek", "li   x12, 1184",
                     "jal  x1, sha3_update",
                     "la   x10, context", "la   x11, output_hash", "jal  x1, sha3_final"])],
         data=["ctx_pad:", "  .zero 0", "", ".balign 32", "input_ek:", "  .zero 1184",
    ...
    dict(op="decap", row="hash_g_reuse", app="mlkem_decap",
         note="decap 的 G(m' ‖ h) = SHA3-512(64 B)：软件 Keccak，1 次 update。",
         srcs=list(XOF01), prelude=[],
         calls=[(1, ["la   x10, context", "li   x11, 64", "jal  x1, sha3_init",
                     "la   x10, context", "la   x11, input_mh", "li   x12, 64",
                     "jal  x1, sha3_update",
                     "la   x10, context", "la   x11, output_K", "jal  x1, sha3_final"])],
         data=["input_mh:", "  .zero 64", "", ".balign 32", "output_K:", "  .zero 64"]),
